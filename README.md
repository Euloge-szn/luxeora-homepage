# LUXEORA Fashion House 🏛️

> *« L'avenir du luxe est digital. LUXEORA est prête. »*

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![Responsive](https://img.shields.io/badge/Responsive-Design-gold)
![GitHub](https://img.shields.io/badge/Dépôt-GitHub-181717?style=flat&logo=github&logoColor=white)
![Status](https://img.shields.io/badge/Status-Livré%2028%20Fév%202026-brightgreen)

---

## 📌 Résumé Exécutif

**LUXEORA Fashion House** est une page d'accueil professionnelle réalisée dans le cadre du **Bloc 1 – Développement Web** du Parcours L1 IFRI de l'Académie de Programmation.

Face à une érosion progressive de ses performances commerciales et à une dépendance structurelle aux réseaux sociaux, LUXEORA engage une transformation profonde de sa présence en ligne.

La solution déployée — un site vitrine haute performance en **HTML5 et CSS3 purs** — constitue le premier jalon d'un écosystème numérique cohérent visant à restaurer l'image de marque, diversifier les canaux d'acquisition et instaurer une culture de la donnée.

| 24h/24 | +40% | 100% |
|--------|------|------|
| Disponibilité de la vitrine | Projection leads qualifiés | Responsive Design |

---

## 🧩 Problématique Stratégique

> *« Comment LUXEORA peut-elle moderniser son identité visuelle et sécuriser sa pérennité commerciale à travers une interface web professionnelle, tout en assurant une transition efficace du trafic physique vers un écosystème numérique cohérent ? »*

Trois enjeux majeurs ont guidé ce projet :

**Enjeu Identitaire** — Positionner LUXEORA comme une marque de référence dans l'univers digital du luxe.

**Enjeu Commercial** — Transformer un trafic passif en leads qualifiés et en clients fidèles.

**Enjeu Technologique** — Bâtir une infrastructure numérique indépendante des algorithmes tiers.

---

## 📊 Analyse SWOT

| 💪 FORCES | ⚠️ FAIBLESSES |
|-----------|--------------|
| Qualité supérieure des produits | Absence de présence web |
| Forte notoriété locale | Dépendance aux réseaux sociaux |
| Positionnement premium | Manque de données clients |

| 🚀 OPPORTUNITÉS | 🎯 MENACES |
|-----------------|-----------|
| Digitalisation du luxe | Concurrence digitale accrue |
| Automatisation marketing CRM | Évolution des comportements |
| Nouveaux segments clients | Risque d'obsolescence |

---

## 💡 Solutions Mises en Œuvre

**01 — Identité Numérique Luxe**
Univers visuel épuré, typographie raffinée et photographies plein cadre reflétant l'excellence de la marque.

**02 — Architecture Responsive**
Mise en page Flexbox garantissant une fluidité irréprochable sur tous les supports contemporains.

**03 — Stratégie de Conversion**
Formulaire newsletter pour capter des leads propriétaires, affranchissant la marque des aléas algorithmiques.

**04 — Levier de Réassurance**
Section témoignages clients ancrant la preuve sociale et levant les freins à l'achat.

---

## 🗂️ Structure du projet

```
LUXEORA Fashion House/
│
├── index.html          → Page principale (structure HTML5 sémantique)
├── style.css           → Feuille de style externe (CSS3)
├── README.md           → Documentation du projet
│
└── assets/
    ├── image/
    │   ├── femme1.jpg … femme8.jpg   (Collection Femme)
    │   └── homme1.jpg … homme7.jpg  (Collection Homme)
    └── logo/
        └── logo.jpeg                 (Logo officiel LUXEORA)
```

---

## 📄 Sections de la page

| # | Section | Description |
|---|---------|-------------|
| 1 | **Header** | Logo, navigation desktop, burger mobile, bouton mode sombre/clair, ticker |
| 2 | **Hero** | Image pleine page, titre impactant, statistiques clés, call-to-action |
| 3 | **Bandeau** | Message fort défilant — *"L'élégance intemporelle, désormais à portée de clic"* |
| 4 | **Produits Phares** | 4 pièces iconiques avec catégorie, nom et prix |
| 5 | **Galerie** | Carrousel horizontal scroll-snap avec 6 visuels de saison |
| 6 | **À propos** | Histoire de la marque, valeurs, images empilées, badge 14 ans |
| 7 | **Promotions** | Soldes −40%, compte à rebours, Capsule Dorée, Cercle Prestige |
| 8 | **Témoignages** | 4 avis clients de hauteur uniforme |
| 9 | **Newsletter VIP** | Club Privé LUXEORA avec 4 avantages membres |
| 10 | **Footer** | Logo, mini newsletter, socials animés, liens, contact, bouton ↑ |

---

## ⚙️ Conception Technique

### HTML5 Sémantique — Clean Code & SEO
L'utilisation de balises sémantiques (`header`, `main`, `section`, `article`, `footer`) constitue le socle du référencement naturel. Les moteurs de recherche indexent en priorité les structures claires, offrant un avantage concurrentiel organique durable.

### CSS3 Flexbox — Zéro Framework
L'absence de dépendances externes (Bootstrap, Tailwind) garantit une performance maximale et une pérennité absolue. Sans risque de rupture de compatibilité, le site reste performant et sécurisé sur le long terme.

### 🎨 Thème Clair / Sombre — CSS `:checked` Hack
Aucun JavaScript. Le basculement est réalisé uniquement avec un `<input type="checkbox">` caché et le sélecteur `:checked` :

```css
.theme-input:checked ~ .page-wrap {
  --bg:   #0D0B09;
  --text: #F0EAE0;
  --gold: #C5A96A;
}
```

### 🍔 Menu Burger Mobile — CSS `:checked` Hack
```css
.menu-input:checked ~ .page-wrap .mobile-nav {
  max-height: 500px;
}
```

### 📐 Typographie Fluide — `clamp()`
```css
.section-title { font-size: clamp(2.2rem, 5vw, 5rem); }
.ht-bold       { font-size: clamp(3rem, 8vw, 8rem);   }
```

### 🎠 Galerie — `scroll-snap-type`
```css
.snap-track { scroll-snap-type: x mandatory; }
.snap-card  { scroll-snap-align: start; }
```

---

## 📅 Déroulement des Travaux

| Date | Étape | Description |
|------|-------|-------------|
| Mer 25 Fév | 🚀 Lancement | Répartition des rôles, création du dépôt GitHub et structure HTML |
| Jeu 26 Fév | ⚙️ Développement | Intégration CSS Flexbox, responsive multi-support |
| Ven 27 Fév | 🔧 Optimisation | Corrections, finalisation du design et des sections |
| Sam 28 Fév | ✅ Livraison | Tests mobiles, finalisation README et dépôt officiel GitHub |

---

## 🎨 Choix de Design

| Élément | Choix | Justification |
|---------|-------|---------------|
| **Direction artistique** | Maison de Mode Éditoriale Lumineuse | Luxe accessible, clair et élégant |
| **Police d'affichage** | Cormorant Garamond | Haute couture, contrastes typographiques |
| **Police de corps** | Josefin Sans | Géométrique, moderne, lisible |
| **Fond principal** | Ivoire chaud `#FAF8F3` | Lumineux, chaleureux, non agressif |
| **Couleur accent** | Or patiné `#9A7040` | Luxe sobre, non clinquant |
| **Mode sombre** | Encre `#0D0B09` | Noir organique profond |

---

## 🚀 Lancer le projet

```bash
git clone https://github.com/Euloge-szn/luxeora-homepage.git
```
Puis ouvrir `index.html` dans un navigateur ou via **VS Code Live Server**.

> ✅ Aucune installation requise — HTML & CSS purs, zéro dépendance.

---

## ✅ Contraintes techniques respectées

- [x] HTML5 sémantique exclusivement
- [x] CSS3 avec feuille de style externe
- [x] Aucun JavaScript ni framework
- [x] Flexbox pour toute la mise en page
- [x] Responsive design — mobile, tablette, desktop
- [x] `clamp()` pour la typographie fluide
- [x] `scroll-snap-type` pour la galerie
- [x] Thème sombre/clair via `:checked` CSS pur
- [x] Burger mobile via `:checked` CSS pur
- [x] Code indenté et organisé

---

## 📊 Critères d'évaluation

| Critère | Poids |
|---------|-------|
| Qualité et structure HTML | 20% |
| Maîtrise du CSS et mise en page | 25% |
| Responsive design | 15% |
| Professionnalisme du design | 15% |
| Qualité du rapport écrit | 15% |
| Clarté et pertinence du Business Plan | 10% |

---

## 👨‍💻 Équipe projet

| Nom | Rôle |
|-----|------|
| **TONATO Océane** | Développement & Intégration |
| **HINVI Basilia** | Design & CSS |
| **HANTO Jordy** | Structure HTML & Responsive |
| **DJEHONNON Geoffroy** | Business Plan & Rapport |
| **SOZAN Euloge** | Chef de projet & GitHub |

**Parcours L1 IFRI — Académie de Programmation**
Projet Académique · Bloc 1 Développement Web · Février 2026

---

> *"L'élégance n'est pas un luxe, c'est un état d'esprit."*
> — LUXEORA Fashion House